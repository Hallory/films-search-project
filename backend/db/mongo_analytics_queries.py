import logging
from db.mongo_client import film_stats_collection, search_log_collection
...
def get_popular_films_stats(limit: int = 10) -> list[dict]:
    try:
        cursor = (
            film_stats_collection.find(
                {"last_seen_at": {"$exists": True}},
                {"film_id": 1, "search_impressions": 1, "_id": 0},
            )
        ...
        logger.exception("MongoDB error while fetching popular films stats")
        return []
...
def get_popular_search_queries(
    ...
    try:
        ...
        logger.exception("MongoDB error while fetching popular search queries")
        return []
...
def get_recent_genre_searches(limit: int = 5) -> list[dict]:
    ...
    try:
        ...
        logger.exception("MongoDB error while fetching recent genre searches")
        return []
